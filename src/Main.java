...
public class Main {
    public static void main(String[] args) {
        ...
        int type = in.getKey();
        if (type == -1) return;
        ArrayList<Line> lines = (ArrayList<Line>) in.getValue().get(0);
        FindWay map = (FindWay) in.getValue().get(1);
        String[] names = (String[]) in.getValue().get(2);
        OutputStreamWriter output = (OutputStreamWriter) in.getValue().get(3);
        switch (type) {
            case 0:
                for (Line it : lines) {
                    try {
